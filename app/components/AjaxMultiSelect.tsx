...
export const AjaxMultiSelect: React.FC<AjaxMultiSelectProps> = ({
    ...
    const componentId = `ajax-multi-select-${uniqueId}`;
            ...
            >
                <div className="flex flex-wrap gap-2 p-2 pr-8 min-h-[42px]">
                    {selectedOptions.map(option => (
                        <div
                            key={option.id}
                            className="flex items-center bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded text-sm"
                        >
                            {option.name}
                            {!disabled && (
                                <button
